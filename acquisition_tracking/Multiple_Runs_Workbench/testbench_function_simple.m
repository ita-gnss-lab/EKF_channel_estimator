function [innovationSum] = testbench_function_simple(configuration, currentRun, terminationCondition, parentFolderName, initialCondition, covar_test)
    clc; close all;
    ...
    % phase estimation.
    channelCovarianceMatrix = 1e-2 * eye(2*q + 1); %0.000001 * eye(1 + q);
    channelCovarianceMatrix(q + 1, q + 1) = 10*1e-2; % 0.001;  
  
    
    % NOTE: I changed from x_k_k to P_k_k_1, because, in fact the
    % initialization uses P[1|0].  1e-1, 0, (50)^2/12, (0.1)^2/12,
    P_k_k_1 = blkdiag((1/configuration.chippingFrequency)^2 / 3, pi^2/3, 0, 0, channelCovarianceMatrix); 
    ...
    for k = 1 : simulationSteps
        ...
        if terminationCondition(x_k_k_1)
