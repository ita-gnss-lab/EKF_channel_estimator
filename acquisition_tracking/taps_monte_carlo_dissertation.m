...
%% Monte Carlo Setup
baseSeed = 26437226;
numMonteCarloRuns = 20;
numberOfSweepPoints = 8;
monteCarloSeeds = baseSeed + (0:numMonteCarloRuns - 1);

numberOfCorrelatorsSweep = linspace(9, 23, numberOfSweepPoints);
...
parameters.simulationSteps = 2000;
...
parameters.tapChannelCount = parameters.q + 1;
...
adoptedNumberOfCorrelators = parameters.numberOfCorrelators;
...
totalMonteCarloTasks = ...
    numel(numberOfCorrelatorsSweep) * numMonteCarloRuns + ...
...
carrierDelayMape = zeros(numel(numberOfCorrelatorsSweep), numMonteCarloRuns);
carrierChannelMape = zeros(numel(numberOfCorrelatorsSweep), numMonteCarloRuns);

for carrierIndex = 1:numel(numberOfCorrelatorsSweep)
    for runIndex = 1:numMonteCarloRuns
        taskName = sprintf("Correlators sweep %d/%d, run %d/%d", ...
            carrierIndex, numel(numberOfCorrelatorsSweep), ...
            runIndex, numMonteCarloRuns);
        fprintf("%s\n", taskName);
        parameters.numberOfCorrelators = numberOfCorrelatorsSweep(carrierIndex);
        parameters.q = (parameters.numberOfCorrelators - 1) / 2;
        parameters.C = 2 * parameters.q + 1;
        parameters.middleSample = parameters.q + 1;
        parameters.tapCount = parameters.q + 1;
...
%% Sweep Plots
plotMeanMapeSweep( ...
    numberOfCorrelatorsSweep, carrierDelayMape, carrierChannelMape, ...
    "Number of Corrrelators Perfprmace", "Correlators", ...
    false, false, parameters.nominalCarrierToNoiseDensityRatio, ...
    parameters);

% Get a list of all currently open figure handles
figHandles = findall(0, 'Type', 'figure');

% Define the base filename (ensure the directory exists)
baseName = 'Figure_';
...
for i = 1:length(figHandles)
    ...
    % Construct filenames
    pngName = sprintf('%s%d.png', baseName, i);
    pdfName = sprintf('%s%d.pdf', baseName, i);
    
    % Save as PNG (300 DPI for high quality)
